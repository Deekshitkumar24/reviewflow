...
export default function AdminIssuesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Platform Issues</h1>
        <p className="text-sm text-gray-500 mt-1">Monitor and resolve issues reported directly by student teams.</p>
      </div>
            ...
            return (
              <div key={issue.id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 shadow-sm transition-all relative overflow-hidden">
                ...
                {issue.resolutionNote && (
                  <div className="mt-3 mb-4 p-3 bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 rounded-lg relative z-10">
                    <p className="text-xs font-semibold text-emerald-800 dark:text-emerald-400 mb-1">Resolution Note:</p>
                    <p className="text-sm text-emerald-700 dark:text-emerald-300">{issue.resolutionNote}</p>
                    {issue.resolvedBy?.fullName && (
                      <p className="text-[10px] text-emerald-600/60 dark:text-emerald-500/50 mt-2 text-right">Resolved by {issue.resolvedBy.fullName}</p>
                    )}
                  </div>
                )}
