...
export async function POST(req: Request) {
  try {
    ...
    // Rate Limiting
    if (!checkRateLimit(eventId, "review-nextsteps")) {
      return NextResponse.json(
    ...
    const systemPrompt = "You are an expert hackathon mentor. Suggest 3-5 concrete actionable next steps. Plain text list, one per line, starting with a dash.";
    ...
    const aiResult = await callGemini({
      systemPrompt,
      userInput,
      routeName: "review-nextsteps",
    });

    if (aiResult.error) {
       await logAIAction({ feature: "AI Review Assistant - Next Steps", input: { eventId }, output: aiResult });
    ...
    await logAIAction({
      ...
      eventId,
      feature: "AI Review Assistant - Next Steps",
    ...
    await logAIAction({ feature: "AI Review Assistant - Next Steps", input: {}, output: { error: true, message: error.message } });
    return NextResponse.json(
